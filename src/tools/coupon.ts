  ...
  server.tool(
    ...
    "Cria um novo cupom de desconto",
    {
      ...
      discountKind: z
        ...
        .describe(
          "Tipo de desconto: PERCENTAGE (porcentagem) ou FIXED (valor fixo)",
        ),
      discount: z
        .number()
        .describe(
          "Valor do desconto (em % para PERCENTAGE ou em centavos para FIXED)",
        ),
      notes: z.string().optional().describe("Descrição sobre o cupom"),
      maxRedeems: z
        .number()
        .default(-1)
        .describe("Quantidade máxima de usos (-1 para ilimitado)"),
    ...
    async (params) => {
      ...
      try {
        ...
        const maxRedeemsText =
          data.maxRedeems === -1 ? "Ilimitado" : `${data.maxRedeems} vezes`;
