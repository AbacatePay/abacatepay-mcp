...
export function registerPaymentLinkTools(server: McpServer) {
  server.tool(
    "createPaymentLink",
    "Cria link de pagamento reutilizável (MULTIPLE_PAYMENTS) com itens de produto já cadastrados.",
    {
      apiKey: apiKeyParam(),
    ...
    async (params, extra) => {
      ...
      try {
        ...
        const d = res.data;
        return {
          content: [{ type: "text", text: `Link: ${d.id}\n${d.url}\nstatus: ${d.status}` }],
        };
      } catch (e) {
        return toolError(e);
      }
  ...
  server.tool(
    "getPaymentLink",
    "Busca link de pagamento por id, externalId, customerId ou method.",
    {
      apiKey: apiKeyParam(),
      id: z.string().optional(),
      externalId: z.string().optional(),
    ...
    async (params, extra) => {
      ...
      try {
        const res = await makeAbacatePayRequest<any>({
          path: `/payment-links/get${buildQuery({
            id: p.id,
            externalId: p.externalId,
            customerId: p.customerId,
            method: p.method,
  ...
  server.tool(
    "deletePaymentLink",
    "Remove um link de pagamento (irreversível).",
    {
      apiKey: apiKeyParam(),
      id: z.string(),
    },
    async (params, extra) => {
      ...
      try {
        const res = await makeAbacatePayRequest<any>({
          path: `/payment-links/delete${buildQuery({ id: p.id })}`,
          apiKey: p.apiKey,
          sessionId: extra.sessionId,
          method: "POST",
          body: "{}",
        });
        return { content: [{ type: "text", text: `Link removido: ${res.data?.id ?? p.id}` }] };
      } catch (e) {
        return toolError(e);
      }
    }
  ...
  server.tool(
    "refundPaymentLink",
    "Reembolsa integralmente um pagamento realizado através de um link. Use o id do pagamento individual, não o id do link.",
    {
      apiKey: apiKeyParam(),
      id: z.string().describe("ID público do pagamento (char_/pix_char_/card_)."),
