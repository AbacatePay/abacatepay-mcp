  ...
  server.tool(
    ...
    "Cria um QR Code PIX para pagamento direto",
    {
      ...
      expiresIn: z
        .number()
        .optional()
        .describe("Tempo de expiração em segundos (opcional)"),
      description: z
        .string()
        .max(140)
        .optional()
        .describe(
          "Mensagem que aparecerá no pagamento PIX (máx 140 caracteres)",
        ),
      customer: z
        .object({
        ...
        .optional()
        .describe("Dados do cliente (opcional)"),
    },
    async (params) => {
      const { apiKey, amount, expiresIn, description, customer } =
        params as any;
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Falha ao criar QR Code PIX: ${
  ...
  server.tool(
    ...
    "Simula o pagamento de um QR Code PIX (apenas em modo desenvolvimento)",
    {
      ...
      metadata: z
        .object({})
        .optional()
        .describe("Metadados opcionais para a requisição"),
    },
    async (params) => {
      ...
      try {
          ...
          content: [
            {
              ...
              text:
                ...
                `${
                  data.devMode
                    ? "⚠️ Simulação realizada em modo de desenvolvimento"
        ...
        return {
          content: [
            {
              type: "text",
              text: `Falha ao simular pagamento PIX: ${
                error instanceof Error ? error.message : "Erro desconhecido"
              }`,
            },
          ],
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Falha ao verificar status do PIX: ${
                error instanceof Error ? error.message : "Erro desconhecido"
              }`,
            },
          ],
